...
pub const Pio = enum(u2) {
    ...
    pub fn sm_exec_set_x(self: Pio, sm: common.StateMachine, value: u32) void {
        // 32 = .bit_count = 0?
        const inst = Instruction(.RP2350){ .tag = .out, .delay_side_set = 0, .payload = .{ .out = .{ .destination = .x, .bit_count = 0 } } };
    ...
    pub fn sm_exec_set_y(self: Pio, sm: common.StateMachine, value: u32) void {
        // 32 = .bit_count = 0?
        const inst = Instruction(.RP2350){ .tag = .out, .delay_side_set = 0, .payload = .{ .out = .{ .destination = .y, .bit_count = 0 } } };
        sm_write(self, sm, value);
        sm_exec(self, sm, inst);
    }

    pub fn sm_exec_set_pindir(self: Pio, sm: common.StateMachine, data: u5) void {
        const inst = Instruction(.RP2350){ .tag = .set, .delay_side_set = 0, .payload = .{ .set = .{
            .data = data,
            .destination = .pindirs,
        } } };
        sm_exec(self, sm, inst);
    ...
    pub fn sm_exec_jmp(self: Pio, sm: common.StateMachine, to_addr: u5) void {
        const inst = Instruction(.RP2350){ .tag = .jmp, .delay_side_set = 0, .payload = .{ .jmp = .{ .address = to_addr, .condition = .always } } };
        sm_exec(self, sm, inst);
    }
};
